...
$$ x_{n+1} = \sqrt{x_n + 2}, x_1 = \sqrt{2} \quad (n = 1,2,3,\dots) $$

$$ 由于 x_{n + 1} - x_n = \sqrt{x_n + 2} - x_n = \frac{x_n + 2 - x_n^2}{\sqrt{x_n + 2} + x_n} = \frac{(2 - x_n)(1 + x_n)}{\sqrt{x_n + 2} + x_n} \quad (式：1)$$

$$ 由 x_1 = \sqrt{2} < 2, 设 x_k < 2 $$

$$ 则 x_{k + 1} = \sqrt{x_k + 2} < \sqrt{2+2} = 2 $$

$$ 则假设成立，即 x_n < 2，即有上界 2 $$

$$ 则，式1 > 0，即 x_{n+1} > x_n，即数列单调增 $$

故数列极限存在（单调有界准则），令极限为A

$$ 在 x_{n + 1} = \sqrt{2 + x_n} 两端取极限(n → \infty) $$

$$ A = \sqrt{2 + A} \Rightarrow A = 2, A = -1 $$
